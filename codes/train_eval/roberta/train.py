...
from data_utils import accuracy, myprocessors, convert_examples_to_features # self-defined
...
################ Some necessary functions for helping raw data into model   ################################
...
def process_examples(args,examples): 
	# this function is just for temporary storage
	example_files  = f"{args.output_dir}/samples.jsonl" 
	with jsonlines.open(example_files,'w') as f:
...
def load_and_cache_examples(args, task, tokenizer, evaluate=False,data_type='training'):
	...
	if evaluate and os.path.exists(cached_features_file):
		...
		# example[0] e.g.
		# {'id': 10793, 'context': 'aching is similar to', 'correct': 0, 'candidates': ['painful.', 
		# 'unshaven.', 'high potential.'], 'keywords': ['aching'], 'dimenison'/'dim':xxx}  # if we have 'dimenison' 

		# write all the results of the examples into a raw data set,  to ensure that the 
		# format is consistent with atomic data format, which is convenient for subsequent analysis
		if data_type == 'training':
			examples = process_examples(args,examples)

		features = convert_examples_to_features(examples, tokenizer, max_length=args.max_seq_length)
		# features[0] e.g.
		# [input_ids, label_ids, correct_answer, dimension,sample_id]
...
def train(args, train_dataset, model, tokenizer, eval_dataset):
	...
	if args.n_gpu > 1:
		print("Let's use", torch.cuda.device_count(), "GPUs by using nn.DataParallel !")
...
def evaluate(args, model, tokenizer, eval_dataset):
	...
	for batch in tqdm(eval_dataloader, desc="Evaluating"):
		...
		# example out_label_ids: [array([0, 2, 2, 1, 1, 2, 0, 1])]
...
def test_test(args, model, tokenizer, train_dataset,epoch):
	...
	eval_dataloader = DataLoader(train_dataset, sampler=eval_sampler, batch_size=args.eval_batch_size, collate_fn=mCollateFn)
	...
	sample_ids = []
	correct_number = []
	for batch in tqdm(eval_dataloader, desc="Mean Cal"):
		model.eval()
		with torch.no_grad():
			num_cand = len(batch[0][0])
			choice_loss = []
	...
	with open(output_stat_file,'w') as f:
		for index, each_sample in enumerate(preds):
			...
			pred_label_index = pred_label_ids[index]
...
def main():
	...
	if args.do_train:
		...
		os.system("cp train.py %s" % os.path.join(args.output_dir, 'run_pretrain.py'))
	...
	import time
	start = time.time()
	eval_dataset = load_and_cache_examples(args, args.task_name, tokenizer, evaluate=True,data_type='eval')
	end = time.time()
	print(f"Time elapse for eva data loading: {end-start}s")
	#ignore initial evaluation (spend too much time!)
	# if args.do_train:
	# 	init_result = evaluate(args, model, tokenizer, eval_dataset)
	# 	print (init_result) => 0,51
	if args.do_train:	
		start = time.time()
		train_dataset = load_and_cache_examples(args, args.task_name, tokenizer, evaluate=False,data_type='training')
		end = time.time()
		print(f"Time elapse for train data loading: {end-start}s")
